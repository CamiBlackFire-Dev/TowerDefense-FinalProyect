...
- `BoardGrid.Move()` returns a `MoveResult` with `TowerMoveEvent`/`TowerMergeEvent` lists (deterministic, used to animate). In Play Mode `BoardManager` animates slides and merge pulses via coroutines (no external tweening); **movement input is ignored while the animation runs**. Outside Play Mode (EditMode tests) visuals update instantly.
